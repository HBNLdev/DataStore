...
def time_proximal_fill_fast(comp_dfj, new_datecol, fup_col, joined_cols,
    ...
    for ID in IDs:
        ...
        for id_i, id_r in ID_df.iterrows():
            ...
            try:
                fup_isnan = np.isnan(id_r[fup_col])
            except TypeError:  # if fup is a string
                fup_isnan = False
            if fup_isnan and (id_r['age'] > min_age) and (id_r['age'] < max_age):
...
def careful_join(comp_df, collection, subcoll=None, do_fill=False,
    ...
    index_levels = join_df.index.names
    for level in index_levels:
        ivals = join_df.index.get_level_values(level)
        if np.nan in ivals:
            print('found a nan in an index!')
            ind_series = pd.Series(ivals)
            ind_series_notnan = ~ind_series.isnull()
            join_df = join_df[list(ind_series_notnan)]
...
def careful_join_ssaga(comp_df, raw=False, do_fill=False, min_age=-np.inf, max_age=np.inf,
                       session_datecol_in='date', followups=None):
    ''' given a compilation dataframe indexed by ID/session (comp_df) with an age column,
        and with a session date column named by session_datecol_in,
    ...
    ssaga_docs = get_colldocs(coll, ssaga_subcoll, query, add_proj)
    cssaga_docs = get_colldocs(coll, cssaga_subcoll, query, add_proj)
